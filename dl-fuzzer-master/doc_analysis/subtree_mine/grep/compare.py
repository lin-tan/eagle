...
def run_cnt(dir, files):
    target = ['dtype','structure','shape', 'tensor_t','ndim','enum','range']
...
def compare(old, new, save=True):
    ...
    group = {
        'prim_dtype': ['dtype'],
        # 'nonprim_dtype': ['tensor_t', 'structure'],
        'nonprim_dtype': [ 'structure', 'tensor_t'],
        'shape': ['shape', 'ndim'],
        'validvalue': ['enum', 'range']
    }
    ...
    def _get_constr(yamlfile, arg):
        ...
        for cat in group:
            ...
            constr[cat] = sorted([str(x) for x in constr[cat]])
        return constr
...
# python compare.py ../constr/tf/success/ ./tf/success/ 
# python compare.py ../constr/pt/success/ ./pt/success/
# python compare.py ../constr/mx/success/ ./mx/success/ 
