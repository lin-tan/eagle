...
anno_list = read_yaml('./preprocess/all_anno.yaml')
...
def update_rule(decoded_subtree, selected_constr):
    new_rule = {}       # the constraints associated with the decoded subtree
...
def single_sent(q, row, all_label_row, min_conf, top_k):
    ...
    for label_row in all_label_row:
    ...
    q.put([decoded_subtree, new_rule])
...
def build(mine_result_path, label_path, save_path, min_conf=0, top_k=-1):
    ...
    for index, row in mine_result_df.iterrows():  # for each frequent subtree
        ...
        assert int(row['Frequency']) == len(unique_sent_idx)
        for sidx in unique_sent_idx:
            # for each tree the subtree comes from
            all_label_row.append(label.iloc[sidx])
        p = Process(target=single_sent, args=(q, row, all_label_row, min_conf, top_k))
        p.start()
        ...
        if pcnt >= MAX_PROC:
            ...
            for p in process_pool:
                decoded_subtree, new_rule = q.get()
                if new_rule:
...
# Duration
# tf: Duratino: 4.524677753448486 seconds
# pt : Duration: 1.752676248550415 seconds
# mx: Duration: 2.908578395843506 seconds