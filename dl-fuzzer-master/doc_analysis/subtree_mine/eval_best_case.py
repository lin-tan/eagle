...
def get_constr_cat(arg_data, target_cat):
    constr_dict = init_constr()
    for tc in target_cat:
        for cat in cat_group[tc]:
            if cat in arg_data: 
                constr_dict[cat] = set(arg_data[cat])       # [IMPORTANT] convert to set
    return constr_dict
...
def main(extracted_folder, label_folder, ):
    ...
    for f in sample_list:
        ...
        for arg in sample_list[f]:
            ...
            if extracted_data:
                extracted_constr = get_constr_cat(extracted_data['constraints'][arg], ['validvalue', 'dtype']) 
            else:
