...
class func_parser():
    def __init__(self, func_src):
        ...
        self.api_name = None
        self.yaml_data = None
        self.no_input = False   # for layer API, allow no input
    ...
    def get_sig(self):
        ...
        if parsed:
            ...
            #print(self.func_src)
            print('warning: signature parsing failed on [%s]' % self.title)
            return ''
    ...
    def init(self, url):
        ...
        if signature and not signature.isspace() :
    ...
    def get_param_sect(self):
        ...
        if not descp_sect and self.no_input==False: # has input but no descp_sect
    ...
    def parse_input_descp(self):
        ...
        for ps in param_sect[:1]:       # only consider the first one
...
class web_parser():
    ...
    def parse_function(self):
        ...
        def _parse_func(func_src, func_url, check_class=True, check_inside_func=True):
            ...
            if get_module_name(func_parser_obj.title) not in module_name:
                raise GoToTheNextOne(func_url , func_parser_obj.title, 'not target module', save=False)
            
            if func_parser_obj.title.lower() in stop_api:
                raise GoToTheNextOne(func_url , func_parser_obj.title, 'container api', save=False)
        ...
        for i, func in enumerate(self.api_class):
...
def main(test_url=None):
    ...
    save_folder = './pt1.5_nn_new/' 
    stat_folder = './stat1.5_new/'
    src_path = read_yaml(stat_folder+'save_src.yaml')       # save_src_new_format.yaml for 1.6 1.7
    module_filename = ['nn.html']
    global module_name
    module_name = ['torch.nn']

    global stop_api
    stop_api = ['torch.nn.module', 'torch.nn.sequential', 
    ...
    for url in candidate:
        ...
        # 1.5
        if filename not in module_filename:
            continue

        # 1.6 1.7
        
        # if get_module_name(filename) not in module_name:
        #     continue
    ...
    save_yaml(stat_folder+'running_stat_class.yaml', running_stat)
    save_yaml(stat_folder+'module_stat_class.yaml', module_stat)
