...
class func_parser():
    ...
    def parse_param_sect(self, param_str):
        ...
        def _save_descp(parsed_varname, parsed_dtype, parsed_descp):
            ...
            if parsed_dtype and not parsed_dtype.isspace():
            ...
            if not self.yaml_data.is_input(parsed_varname,allow_inconsistent_when_kwargs=True, ignore_star =True):
                if parsed_varname in ['Inputs', 'Outputs', 'Output']:
                    print('warning: %s detected in %s: %s' % (parsed_varname, self.title, self.url))
                else:
...
class web_parser():
    ...
    def parse_function(self):
        ...
        def _parse_func(func_src, check_class=True, check_inside_func=True):
            ...
            #     return
            if get_module_name(func_parser_obj.title).lower() not in ['mxnet.gluon.contrib.nn', 'mxnet.gluon.contrib.rnn', 'mxnet.gluon.nn', 'mxnet.gluon.rnn']:
                raise GoToTheNextOne(self.parent_url , func_parser_obj.title, 'not target function', save=False)
            ...
            if func_parser_obj.title not in self.func_list:
                if func_parser_obj.title in self.deprecated_func_list:
                    ...
                    print('warning: Func [%s] not found: %s' % (func_parser_obj.title, self.parent_url))
        ...
        for i, func in enumerate(self.api_class):
            ...
            try:
                for method in func.find_all(class_='method'):
                    method.decompose()
...
def main(test_url=None):
    ...
    save_folder = './mx1.6_nn_new/' 
    src_path = read_yaml('./stat1.6_new/save_src_class.yaml')
    func_info = read_yaml('./stat1.6_new/mxnet_1.6_fromweb_class.yaml')
    ...
    save_yaml('./stat1.6_new/running_stat_class.yaml', running_stat)
    save_yaml('./stat1.6_new/module_stat_class.yaml', module_stat)
