...
from constant import MAX_NUM_DIM, Status, MAX_PERMUTE, NAN_EXIT, ZERO_DIV_EXIT, SYS_EXIT, INCON_EXIT
from errors import FuzzerError
from EquivParam import Param
...
class Fuzzer:
    ...
    def _run_max_iter(self, max_iter, obey, gen_script=False):
        ...
        while True:
            ...
            try:
                # generate new inputs
                new_seed = self._generate_new_input(
                    obey)  # self._generate(seeds, obey=obey, reduce_failure=self.fstatus.reduce_failure)
    ...
    def _pick_disobey_param(self, param_list, obey):
        ...
        candidate = list(
            set(disobey_params) & set(param_list))  # intersection of two list (param can disobey & param to fuzz)
    ...
    def _pick_mutate_param(self, param_list):
        if util.random_prob(self.config.mutate_p):
            ...
            candidate = list(
                set(mutate_all) & set(param_list))  # intersection of two list (param can disobey & param to fuzz)
            if candidate:
    ...
    def _test(self, seed, count=None, obey=True, gen_script=False, save=False):
        ...
        if self.config.model_test:
            while True:
                try:
                    ...
                    model_input_path, input_duplicate = util.save_seed_to_file(model_input, self.config.workdir, count,
                                                                               save_to_file='gen_order_model_input',
                                                                               file_type='model input', save=save)
                    break
                except FuzzerError as e:
        ...
        p = mp.Process(target=worker_func,
                       args=(seed, model_input, res_queue, seed_fpath, exception_list, self.config.model_test, count))
        p.start()
        ...
        #                                               self.config.package,
        #                                               self.target_func_statement)
        if (p.exitcode < 0 and -p.exitcode != signal.SIGTERM) or p.exitcode == NAN_EXIT or p.exitcode == ZERO_DIV_EXIT or p.exitcode == SYS_EXIT or p.exitcode == INCON_EXIT:
    ...
    # @tf.function()
    # def _tf_optimized_func(self, func, seed):
    #     return func(**seed)
    #
    ...
    #
    #         ret_tf_optimized = self._tf_optimized_func(self._target_func, seed)
    #
    ...
    #         # sys.exit(NAN_EXIT)
    #
    #     if not np.allclose(ret, ret_tf_optimized):
    #         print("Found difference!")
    #         print(np.max(np.abs(ret - ret_tf_optimized)))
    #         raise CustomCrash(INCON_EXIT)
    #
    #     return ret
    ...
    def _expect_exception(self, seed, model_input, res_queue, seed_fpath='', exception_list=[], model_test=False,
                          count=None):
        had_exception = False
        constructor_fail = True
        try:
